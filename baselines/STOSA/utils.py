...
def split_data(user_seq):
    len_split = []
    for seq_temp in user_seq:
        if len(seq_temp) > 4:
            for star in range(len(seq_temp)-3):
                len_split.append(seq_temp[:])
            else:
                len_split.append(seq_temp)
    return len_split


def get_data_from_pkl(path_data):
    with open(path_data, 'rb') as f:
        data_raw = pickle.load(f)
    seqs = []
    for id_temp in data_raw['train']:
        seq_temp = data_raw['train'][id_temp] + data_raw['val'][id_temp] + data_raw['test'][id_temp]
        seqs.append(seq_temp)
    # seqs = split_data(seqs)
    
    num_items = len(data_raw['smap'])
    # num_users = len(data_raw['umap'])
    num_users = len(seqs)
    valid_rating_matrix = generate_rating_matrix_valid(seqs, num_users, num_items+2)
    test_rating_matrix = generate_rating_matrix_test(seqs, num_users, num_items+2)
    # return seqs, max_item, num_users
    return seqs, num_items, valid_rating_matrix, test_rating_matrix, num_users
